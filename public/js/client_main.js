...
let showing_player_sheet = false;
let urlToLoad = null;
const urlParams = new URLSearchParams(window.location.search);
if ([...urlParams].length > 0) {
    urlParams.forEach((value, key) => {
        console.log(`URL Parameter: ${key} = ${value}`);
        showing_player_sheet = true;
        urlToLoad = `${key}`;

    });
}

if (!showing_player_sheet) {
...
if (!showing_player_sheet) {
    ...
    editMap.style.visibility = "hidden";
}
...
// don't upload
// TODO: not just to threed window
